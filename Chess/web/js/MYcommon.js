...
com.init = function (stype) {
    ...
    com.name = decodeURI(loc.substr(n2 + 1, n1 - n2));//从=号后面的内容,获取用户名
    ...
    while (isOffensive===1){
        xmlhttp.onreadystatechange=function(){
            if (xmlhttp.readyState===4 && xmlhttp.status===200){
                if(Number(xmlhttp.responseText)==3){
                    break;
                }
            }
            sleep(3000);
            alert("等待了3s,"+xmlhttp.responseText);
        };
        xmlhttp.open("GET","/match?name="+com.name+"&&next_step=-2&&isregret=0&&me="+isOffensive,false);
        xmlhttp.send();
    }
}
...
function loadXMLDoc(){
    ...
    xmlhttp.onreadystatechange=function(){
        if (xmlhttp.readyState==4 && xmlhttp.status==200){
            if(Number(xmlhttp.responseText)==2)//自己成为黑方
            {
                isOffensive=0;
                alert("com.isOffensive:"+isOffensive);
            }
            if(Number(xmlhttp.responseText)==1)//自己成为红方
            {
                isOffensive=1;
                alert("com.isOffensive:"+isOffensive);
            }
            //alert("xmlhttp.responseText:"+Number(xmlhttp.responseText));
            //wait_player();
            //if(isOffensive===0){
            //     com.args=com.args2;
            // }
    ...
    xmlhttp.open("GET","/match?name="+com.name+"&&next_step=-1&&isregret=0&&me="+isOffensive,true);
    xmlhttp.send();
}
...
com.args2 = {
    ...
    'x': { text: "相", img: 'b_x', my: 1, bl: "x", value: com.value.x },
    's': { text: "仕", img: 'b_s', my: 1, bl: "s", value: com.value.s },
    'j': { text: "将", img: 'b_j', my: 1, bl: "j", value: com.value.j },
    'p': { text: "炮", img: 'b_p', my: 1, bl: "p", value: com.value.p },
    'z': { text: "兵", img: 'b_z', my: 1, bl: "z", value: com.value.z },

    //蓝子
    'C': { text: "車", img: 'r_c', my: -1, bl: "c", value: com.value.C },
    'M': { text: "馬", img: 'r_m', my: -1, bl: "m", value: com.value.M },
    'X': { text: "象", img: 'r_x', my: -1, bl: "x", value: com.value.X },
    'S': { text: "士", img: 'r_s', my: -1, bl: "s", value: com.value.S },
    'J': { text: "帅", img: 'r_j', my: -1, bl: "j", value: com.value.J },
    'P': { text: "炮", img: 'r_p', my: -1, bl: "p", value: com.value.P },
    'Z': { text: "卒", img: 'r_z', my: -1, bl: "z", value: com.value.Z }
};
